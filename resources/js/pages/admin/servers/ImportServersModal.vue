...
// Selection State
const selectedVmids = ref<Set<string>>(new Set()); // unique key: "nodeId-vmid"
...
const importMutation = useMutation({
    mutationFn: async () => {
        ...
        const promises = serversToImport.map(vm => {
            // Sanitize hostname from name logic
            const sanitizeHostname = (name: string) => name.toLowerCase().replace(/[^a-z0-9-]/g, '-');
            ...
            return adminServerApi.create({
                ...
                vmid: vm.vmid,
                cpu: vm.cpu,
                memory: Math.round(vm.memory * 1024 * 1024 * 1024), // GB to Bytes
                disk: Math.round(vm.disk * 1024 * 1024 * 1024),     // GB to Bytes
                bandwidth_limit: 0, // Default to 0 (unlimited) instead of null to match type
            });
        });
            ...
            <div class="fixed inset-0 z-10 overflow-y-auto">
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    <TransitionChild as="template" enter="ease-out duration-300"
                        ...
                        <DialogPanel
                            class="relative transform overflow-hidden rounded-xl bg-secondary-900 border border-secondary-800 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-4xl">
                            ...
                            <div class="px-6 py-6">
                                ...
                                <div v-else-if="unmanagedServers.length === 0"
                                    ...
                                    <p class="text-sm mt-1 text-secondary-500">All VMs are already imported or nodes are
                                        unreachable.</p>
                                </div>

                                <div v-else class="space-y-4">
                                    <p class="text-sm text-secondary-400">Found {{ unmanagedServers.length }} unmanaged
                                        VMs. Select servers to import.</p>

                                    <div
                                        class="border border-secondary-800 rounded-lg overflow-hidden bg-secondary-900/50">
                                        <table class="w-full text-left text-sm">
                                            ...
                                            <tbody class="divide-y divide-secondary-800/50 text-secondary-200">
                                                <tr v-for="server in unmanagedServers" :key="server.uniqueId"
                                                    ...
                                                    <td class="p-3">
                                                        <span
                                                            class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-secondary-800 text-xs">
                                                            <CpuChipIcon class="w-3 h-3" /> {{ server.nodeName }}
                                                        </span>
                                                    </td>
                                                    <td class="p-3">
                                                    ...
                                                    <td class="p-3 text-xs text-secondary-400">
                                                        {{ server.cpu }}c / {{ server.memory }}G / {{ server.disk }}G
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    ...
                                    <!-- Import Controls -->
                                    <div
                                        class="flex items-end gap-4 p-4 bg-secondary-800/20 rounded-lg border border-secondary-800/50">
                                        <div class="flex-1">
