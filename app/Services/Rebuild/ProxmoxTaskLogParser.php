...
class ProxmoxTaskLogParser
{
    public function parseCloneProgress(array $logs): ?array
    {
        ...
        foreach ($logs as $line) {
            ...
            if (preg_match('/transferred\s+([\d.]+)\s+([A-Za-z]+)\s+of\s+([\d.]+)\s/i', $lineData, $matches)) {
    ...
    private function transferToBytes(float $value, string $unit): int
    {
        return match(strtoupper($unit)) {
            'B' => (int) $value,
            'KB', 'KIB' => (int) ($value * 1024),
            'MB', 'MIB' => (int) ($value * 1024 * 1024),
            'GB', 'GIB' => (int) ($value * 1024 * 1024 * 1024 * 1024),
            'TB', 'TIB' => (int) ($value * 1024 * 1024 * 1024),
            default => (int) $value,
        };
    }
